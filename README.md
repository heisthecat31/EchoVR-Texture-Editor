...
This program can be opened as python script or through the .exe, DO NOT OPEN evrModifyFiles.exe as it is only there to be used by the editor program and do not close the cmd which opens when extracting(cmd only shows when using the .exe version).
