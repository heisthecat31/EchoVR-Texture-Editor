...
class EchoVRTextureViewer:
    ...
    def auto_setup_folders(self):
        ...
        # Define folder paths
        self.extracted_folder = os.path.join(script_dir, "extracted")
        self.input_folder = os.path.join(script_dir, "input")
        self.output_folder = os.path.join(script_dir, "extracted")  # Output same as extracted
        self.newdatafolder = os.path.join(script_dir, "newdatafolder")
        
        # Create folders if they don't exist
        for folder in [self.extracted_folder, self.input_folder, self.newdatafolder]:
        ...
        print(f"Auto-setup folders:")
        print(f"  Extracted: {self.extracted_folder}")
        print(f"  Input: {self.input_folder}")
        print(f"  Output: {self.output_folder}")
        print(f"  New Data: {self.newdatafolder}")
    
    def auto_find_echo_vr_data(self):
        """Automatically find Echo VR data folder"""
        echo_vr_paths = [
            r"C:\Program Files\Oculus\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"C:\Program Files\Oculus\Software\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"D:\Program Files\Oculus\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"D:\Program Files\Oculus\Software\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"G:\Program Files\Oculus\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"G:\Program Files\Oculus\Software\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"E:\Program Files\Oculus\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"E:\Program Files\Oculus\Software\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"F:\Program Files\Oculus\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
            r"F:\Program Files\Oculus\Software\Software\ready-at-dawn-echo-arena\_data\5932408047\rad15\win10",
        ]
        
        for path in echo_vr_paths:
            if os.path.exists(path):
                self.set_data_folder(path)
                self.log_info(f"✓ Auto-detected Echo VR data: {path}")
                return
        
        # If not found, check config
        if self.config.get('data_folder') and os.path.exists(self.config['data_folder']):
            self.set_data_folder(self.config['data_folder'])
            self.log_info(f"✓ Using configured data folder: {self.config['data_folder']}")
        else:
            self.log_info("⚠ Echo VR data folder not found automatically")
    ...
    def setup_ui(self):
        ...
        self.status_label = tk.Label(main_frame, text="Auto-setting up folders...",
        ...
        self.data_folder_label = ttk.Label(evr_frame, text="Searching for Echo VR...", foreground=self.colors['text_muted'])
        ...
        self.extracted_folder_label = ttk.Label(evr_frame, text="Auto-set to 'extracted'", foreground=self.colors['success'])
        ...
        # Update status with auto-set folders
        self.status_label.config(text="Folders auto-set: extracted, input, newdatafolder")
    ...
    def filter_textures(self, event=None):
        ...
        self.status_label.config(text=f"Found {len(self.filtered_textures)} textures (filtered from {len(self.all_textures)})")
    ...
    def repack_package(self):
        ...
        # Use newdatafolder as the output directory
        output_dir = self.newdatafolder
